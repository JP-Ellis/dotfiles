...
-   When performing validations, use early returns to reduce nesting and improve readability.
    ...
    -   For project-specific issues, define and use custom exceptions to provide clear and meaningful error handling. These should inherit from a base error class specific to your project.
...
-   References to other functions, classes, or modules must be linked, using the fully qualified Python path:
    ...
    ```markdown
    A link to a [`ClassName.method`][package.module.ClassName.method] or a
    [`function_name`][package.module.function_name].
    ```
