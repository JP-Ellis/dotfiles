...
#+end_src
** Astro
Doom has no Astro module, and ~.astro~ files are not mapped to any mode by
default. This provides a tree-sitter major mode for them.
#+begin_src emacs-lisp
...
#+end_src
** Svelte
The ~web~ module only hands ~.svelte~ files to ~web-mode~ as plain HTML, which
loses the script and style blocks.
#+begin_src emacs-lisp
(package! svelte-mode
